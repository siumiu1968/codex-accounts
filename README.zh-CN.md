...
- 内置 OpenCodex 2.29.0，仅供独立的 `opencodex-lab` Profile 使用。
- 固定使用本地端口 `10100`；端口被占用时安全停止，不会自动换端口。
- 普通 Profile 保持 OpenAI 直连或原有自定义路由。
- Lab 可选择加入同一套本地共享对话；登录、设置、本地记忆和代理路由保持独立。
- Dashboard 每次取得新的短期本地 session，无需输入自动生成的管理员令牌。
- 使用 OpenCodex 自带的繁体中文界面，不再打包旧版香港繁体 overlay。

## 2.7.3 更新

- 每分钟低成本检查内置空间，少于 10 GB 时自动清理子 task，恢复至约 20 GB 后停止。
- 只处理 agent 创建、已完成或长时间停止更新的旧子 task。
- 保留主 task、每组最新子 task、用户创建的 task、仍被进程使用的数据，以及所有登录、Token 和设置。
- 清理记录设有大小上限，避免日志本身持续占用存储空间。

## 2.7.2 更新

- 加入冷热分层存储，可把旧 task 批量冷藏到 ORICO，并支持批量还原、七天以上与全选。
- 通过 task 活跃状态、文件锁与事务锁，避免移动正在使用的数据。
- 正式账号可使用各自 OpenAI 登录直接连接，不再依赖本地 `10100` 代理。
- 子进程输出限制为最多 8 MB，避免 command output、quota refresh 或工具日志令内存无限增长。
- 保留 2.7.1 的 GitHub API／网页双路径更新检查。
...
- 内置 OpenCodex；仅独立 Lab 使用本地代理，可选择共享本地对话，同时保留逐 Profile 登录、设置与路由隔离。
